...
def app_storage_path(filename: str) -> str:
    """
    Store live data beside the script during normal Python runs and beside the EXE
    during PyInstaller builds. Do not use _MEIPASS for writable data.
    """
...
DB_FILE = app_storage_path(DB_NAME)
...
def initialize_database() -> None:
    ...
    This app originally used a 5-column CSV row:
    Work Order, Serial Number, Status, Notes, Timestamp

    The SQLite layer keeps the UI compatible with that shape, while also tolerating
    the earlier import schema that used created_at/updated_at and a separate parts column.
    """
...
def db_columns() -> Set[str]:
    initialize_database()
    with get_db_connection() as conn:
        return {row["name"] for row in conn.execute("PRAGMA table_info(cases)").fetchall()}


def db_timestamp_column(columns: Optional[Set[str]] = None) -> str:
...
def case_row_to_app_row(row: sqlite3.Row) -> List[str]:
    data = dict(row)
    work_order = str(data.get("work_order") or data.get("Work Order") or "").strip()
    serial_number = str(data.get("serial_number") or data.get("Serial Number") or "").strip().upper()
    status = str(data.get("status") or data.get("Status") or "Ordered").strip()
    raw_notes = str(data.get("notes") or data.get("Notes") or "").strip()
    parts_value = str(data.get("parts") or data.get("Parts") or "").strip()
    timestamp = str(
        data.get("timestamp")
    ...
    # If notes are already in the app's structured format, keep them.
    if raw_notes.startswith("Parts:"):
        notes = raw_notes
    elif parts_value:
        selected_parts = []
        other_part = ""
        for part in [p.strip() for p in parts_value.split(",") if p.strip()]:
            if part in PART_OPTIONS:
                selected_parts.append(part)
            elif part.lower().startswith("other:"):
                other_part = part.split(":", 1)[1].strip()
            else:
                other_part = part if not other_part else f"{other_part}, {part}"
        notes = build_notes_field(selected_parts, other_part, raw_notes)
    else:
        notes = raw_notes
...
def insert_app_row(row: List[str]) -> None:
    if len(row) < 5:
        return

    work_order, serial_number, status, notes, timestamp = row[:5]
    columns = db_columns()

    insert_values = {}
    if "work_order" in columns:
        insert_values["work_order"] = work_order
    if "serial_number" in columns:
        insert_values["serial_number"] = serial_number.upper()
    if "status" in columns:
        insert_values["status"] = status
    if "notes" in columns:
        insert_values["notes"] = notes

    selected_parts, other_part, user_notes = parse_notes_field(notes)
    if "parts" in columns:
        insert_values["parts"] = build_parts_display(selected_parts, other_part)
    if "created_at" in columns:
        insert_values["created_at"] = timestamp
    if "updated_at" in columns:
        insert_values["updated_at"] = timestamp
    if "timestamp" in columns:
    ...
    sql = f"INSERT INTO cases ({', '.join(col_names)}) VALUES ({placeholders})"
    with get_db_connection() as conn:
        conn.execute(sql, [insert_values[col] for col in col_names])
        conn.commit()
...
def create_database_backup() -> Optional[str]:
    ...
    backup_path = os.path.join(BACKUP_DIR, f"lenovo_tracker_backup_{stamp}.db")
...
def normalize_csv_row(row: List[str]) -> Optional[List[str]]:
    """
    Normalize a legacy CSV row into the app's 5-column internal shape:
    Work Order, Serial Number, Status, Notes, Timestamp

    This is intentionally permissive so older/partial exports are not skipped.
    """
    if not row:
        return None
    ...
    work_order = row[0] if len(row) > 0 else ""
    serial_number = row[1].upper() if len(row) > 1 else ""
    status = row[2] if len(row) > 2 and row[2] else "Ordered"

    # Support both formats:
    # 5-col legacy/internal: Work Order, Serial Number, Status, Notes, Timestamp
    # 6-col display export: Work Order, Serial Number, Status, Parts, Notes, Timestamp
    if len(row) >= 6:
        parts_display = row[3]
        user_notes = row[4]
        timestamp = row[5] or current_timestamp()

        selected_parts = []
        other_part = ""
        for part in [p.strip() for p in parts_display.split(",") if p.strip()]:
            if part in PART_OPTIONS:
                selected_parts.append(part)
            elif part.lower().startswith("other:"):
                other_part = part.split(":", 1)[1].strip()
        ...
        timestamp = row[4] if len(row) > 4 and row[4] else current_timestamp()
...
def normalize_csv_row_from_header(row: List[str], headers: List[str]) -> Optional[List[str]]:
    """Normalize CSV rows using header names when available."""
    if not row:
        return None

    values = {headers[i].strip().lower(): str(row[i]).strip() for i in range(min(len(headers), len(row)))}
    ...
    work_order = first_value("work order", "work_order", "wo", "ticket", "case")
    serial_number = first_value("serial number", "serial_number", "serial", "sn").upper()
    status = first_value("status") or "Ordered"
    timestamp = first_value("timestamp", "updated_at", "created_at", "date") or current_timestamp()

    notes_value = first_value("notes", "note")
    parts_value = first_value("parts", "part")

    if notes_value.startswith("Parts:"):
        notes = notes_value
    elif parts_value:
        selected_parts = []
        other_part = ""
        for part in [p.strip() for p in parts_value.split(",") if p.strip()]:
            if part in PART_OPTIONS:
                ...
                other_part = part if not other_part else f"{other_part}, {part}"
        notes = build_notes_field(selected_parts, other_part, notes_value)
    else:
        notes = notes_value
...
class EditEntryDialog(QDialog):
    ...
    def save_edits(self) -> None:
        ...
        new_serial = self.serial_edit.text().strip().upper()
        ...
        old_key = (self.original_row[0].strip(), self.original_row[1].strip().upper())
        ...
        self.parent_window.update_case_by_key(
            old_key=old_key,
            new_row=[new_work_order, new_serial, new_status, new_notes, new_timestamp],
        )
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # Put the full application body inside a scroll area so controls do not
        # disappear when the window is resized, docked, projected, or opened on
    ...
    def read_all_rows(self) -> List[List[str]]:
        initialize_database()
        rows = [["Work Order", "Serial Number", "Status", "Notes", "Timestamp"]]
        timestamp_col = db_timestamp_column()

        with get_db_connection() as conn:
            query = f"""
                SELECT *
                FROM cases
                ORDER BY COALESCE({timestamp_col}, '') DESC, id DESC
            """
            for db_row in conn.execute(query).fetchall():
                app_row = case_row_to_app_row(db_row)
                if app_row[0] or app_row[1]:
                    rows.append(app_row)
    ...
    def update_case_by_key(self, old_key: Tuple[str, str], new_row: List[str]) -> int:
        work_order, serial_number, status, notes, timestamp = new_row[:5]
        columns = db_columns()
        timestamp_col = db_timestamp_column(columns)

        set_parts = ["work_order = ?", "serial_number = ?", "status = ?", "notes = ?", f"{timestamp_col} = ?"]
        values = [work_order, serial_number.upper(), status, notes, timestamp]

        if "parts" in columns:
            selected_parts, other_part, _user_notes = parse_notes_field(notes)
            set_parts.append("parts = ?")
            values.append(build_parts_display(selected_parts, other_part))

        values.extend([old_key[0], old_key[1].upper()])
        ...
        with get_db_connection() as conn:
            cursor = conn.execute(
                ...
                UPDATE cases
                SET {', '.join(set_parts)}
                WHERE TRIM(work_order) = ? AND UPPER(TRIM(serial_number)) = ?
                """,
                values,
            )
            conn.commit()
    ...
    def entry_exists(self, work_order: str, serial_number: str, exclude_key=None) -> bool:
        target_key = (work_order.strip(), serial_number.strip().upper())
        normalized_exclude = None
        if exclude_key:
            normalized_exclude = (exclude_key[0].strip(), exclude_key[1].strip().upper())

        for row in self.read_all_rows()[1:]:
            if len(row) < 2:
                continue
            key = (row[0].strip(), row[1].strip().upper())
            if normalized_exclude and key == normalized_exclude:
                continue
            if key == target_key:
                return True
        return False
    ...
    def handle_add_entry(self) -> None:
        ...
        notes = build_notes_field(selected_parts, other_part, user_notes)
        insert_app_row([work_order, serial_number, status, notes, current_timestamp()])

        self.display_log()
        self.update_dashboard()
        self.clear_add_entry_form()
        self.set_status_message(f"Added entry {work_order} / {serial_number} to SQLite database.")
    ...
    def handle_update_status(self) -> None:
        ...
        key_set = set((wo.strip(), serial.strip().upper()) for wo, serial in selected)
        ...
        with get_db_connection() as conn:
            updated_count = 0
            for work_order, serial_number in key_set:
                cursor = conn.execute(
                    f"""
                    UPDATE cases
                    ...
                    WHERE TRIM(work_order) = ? AND UPPER(TRIM(serial_number)) = ?
                    """,
                    (new_status, timestamp, work_order, serial_number),
                )
        ...
        self.update_dashboard()
        self.update_status_combo.setCurrentIndex(-1)
        self.set_status_message(f"Updated {updated_count or len(selected)} entr{'y' if len(selected) == 1 else 'ies'} to {new_status}.")
    ...
    def handle_delete_entry(self) -> None:
        ...
        self.display_log()
        self.update_dashboard()
        self.set_status_message(f"Deleted {deleted_count or len(selected)} entr{'y' if len(selected) == 1 else 'ies'} from SQLite database.")
    ...
    def open_edit_window_for_selection(self) -> None:
        ...
        target_key = (selected[0][0].strip(), selected[0][1].strip().upper())
        for row in self.read_all_rows()[1:]:
            if len(row) >= 5 and (row[0].strip(), row[1].strip().upper()) == target_key:
                dialog = EditEntryDialog(self, row)
                dialog.exec()
                return

        QMessageBox.warning(self, "Data Error", "Could not locate the selected entry in the SQLite database.")

    def handle_edit_entry(self) -> None:
        self.open_edit_window_for_selection()

    def table_search_matches(self, query: str) -> List[int]:
        """Return visible table rows matching the search query."""
        ...
        for row in range(self.table.rowCount()):
            ...
            for col in range(self.table.columnCount()):
                ...
                # Remove visual-only markers so searching timestamp/notes remains clean.
                values.append(text.replace("⚠", "").replace("• follow-up", "").lower())

            if query in " ".join(values):
                matches.append(row)

        return matches

    def select_search_match(self, matches: List[int]) -> None:
        """Cycle to the next visible match and scroll it into view."""
        next_row = matches[0]
        ...
        first_item = self.table.item(next_row, 0)
    ...
    def search_log(self) -> None:
        ...
        matches = self.table_search_matches(query)

        # If the row exists but is hidden by status/part filters or the Complete toggle,
        # clear those filters once and search again. This keeps SQLite-backed search from
        # feeling "broken" when the data is present but not currently visible.
        if not matches:
            filters_changed = False
            if self.status_filter_combo.currentText() != "All":
                self.status_filter_combo.blockSignals(True)
                self.status_filter_combo.setCurrentText("All")
                self.status_filter_combo.blockSignals(False)
                filters_changed = True
            if self.part_filter_combo.currentText() != "All":
                self.part_filter_combo.blockSignals(True)
                self.part_filter_combo.setCurrentText("All")
                self.part_filter_combo.blockSignals(False)
                filters_changed = True
            if not self.show_complete_entries:
                self.show_complete_entries = True
                self.toggle_complete_button.setText("Hide Complete Entries")
                filters_changed = True
        ...
        self.select_search_match(matches)
    ...
    def export_to_csv(self) -> None:
        ...
        QMessageBox.information(self, "Export Successful", f"SQLite data exported successfully to {path}")
        self.set_status_message("CSV exported from SQLite successfully.")
    ...
    def import_from_csv(self) -> None:
        ...
        try:
            ...
            if not rows:
                ...
                self.display_log()
                self.update_dashboard()
                msg = "Imported empty CSV. SQLite table cleared."
                if backup_path:
                    msg += f"\nDatabase backup created: {backup_path}"
                QMessageBox.information(self, "Import Successful", msg)
                self.set_status_message("Imported empty CSV; database backed up first.")
                return

            start_index = 0
            headers = []
            first_row_joined = ",".join(cell.strip().lower() for cell in rows[0])
            header_markers = ["work order", "serial number", "serial", "status", "notes", "timestamp", "updated_at"]
            if any(marker in first_row_joined for marker in header_markers):
            ...
            for line_number, row in enumerate(rows[start_index:], start=start_index + 1):
                normalized = normalize_csv_row_from_header(row, headers) if headers else normalize_csv_row(row)
                if normalized:
            ...
            msg = f"Imported {len(imported_rows)} {'entry' if len(imported_rows) == 1 else 'entries'} into SQLite."
            if skipped_rows:
                msg += f"\nSkipped {len(skipped_rows)} blank/unusable row(s). First skipped line(s): {', '.join(map(str, skipped_rows[:10]))}"
            if backup_path:
                msg += f"\nDatabase backup created: {backup_path}"
            QMessageBox.information(self, "Import Successful", msg)
            self.set_status_message(f"CSV imported into SQLite: {len(imported_rows)} imported, {len(skipped_rows)} skipped.")
        except Exception as e:
            QMessageBox.critical(self, "Import Error", f"Failed to import CSV into SQLite:\n{str(e)}")
